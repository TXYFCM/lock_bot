...
def parse_output_to_markdown(output, max_rows_per_msg=12):
    ...
    for line in lines:
        ...
        if m1:
            ...
            rows.append("| {} | <font color=\"{}\">{}</font> | {} | {} | {} |".format(
                ip, color, status, mem.strip(), util.strip(), container.strip() or "-"
            ))
            continue
        ...
        if m2:
            ip = m2.group(1)
            rows.append("| {} | <font color=\"orange\">SSH无法连接</font> | - | - | - |".format(ip))
            continue
        ...
        if m3:
            ip = m3.group(1)
            rows.append("| {} | <font color=\"orange\">网络不通</font> | - | - | - |".format(ip))
            continue

        m4 = re.match(r'^\[([\d\.]+)\] Execution exceeded', line)
        if m4:
            ip = m4.group(1)
            rows.append("| {} | <font color=\"orange\">SSH连接超时</font> | - | - | - |".format(ip))
            continue
