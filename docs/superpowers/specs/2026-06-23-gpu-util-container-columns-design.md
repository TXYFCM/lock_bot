...
> 注：本设计同时把列名规范为 `IP`（原「IP地址」）与 `lock同学`（原「使用者」），带参 query 的 5 列表头也一并改名以保持与纯 AT 7 列表头一致。
...
- **`None`（带参 query）**：5 列表头（改名后）`| IP | 节点状态 | 卡状态 | lock同学 | 剩余时间 |`，不触发 SSH。
- **非 None（纯 AT）**：7 列表头：
...
- `zh.py`：把 `query.table_header` 的 5 列改名为 `| IP | 节点状态 | 卡状态 | lock同学 | 剩余时间 |`；新增 7 列表头变体键（如 `query.table_header_xpu`）：`| IP | 节点状态 | 卡状态 | lock同学 | 剩余时间 | 利用率 | 容器名 |`。
